...
def compose_output(
    ...
    audio_duration = probe_duration(FFPROBE_COMMAND, source_audio) if selected_mode != "replace" else 0.0
    ...
    if selected_mode == "replace":
        chunks = build_replace_chunks(source_audio, segments, config, temp_dir, step_callback=step)
    else:
        chunks = build_interleave_chunks(source_audio, segments, config, temp_dir, audio_duration=audio_duration, step_callback=step)
    if not chunks:
    ...
    output = concat_audio(FFMPEG_COMMAND, chunks, output_path, config.compose.output_bitrate)
    if progress_callback is not None:
...
def build_interleave_chunks(
    ...
    chunks: list[Path] = []
    cursor = 0.0
    resolved_audio_duration = audio_duration if audio_duration is not None else probe_duration(FFPROBE_COMMAND, source_audio)
    for index, segment in enumerate(sorted(segments, key=lambda item: item.start)):
        ...
        if segment.status == "completed" and segment.tts_audio_path and Path(segment.tts_audio_path).exists():
...
def build_replace_chunks(
    ...
    for index, segment in enumerate(sorted(segments, key=lambda item: item.start)):
        ...
        if segment.status == "completed" and segment.tts_audio_path and Path(segment.tts_audio_path).exists():
            ...
            create_silence(FFMPEG_COMMAND, fallback, int((segment.end - segment.start) * 1000))
...
def _count_chunk_steps(selected_mode: str, segments: list[SegmentRecord], audio_duration: float) -> int:
    if selected_mode == "replace":
...
def _count_interleave_steps(segments: list[SegmentRecord], audio_duration: float) -> int:
    steps = 0
    cursor = 0.0
